...
for file in file_list:
	if 'coadd.cat' in file:
		coords.append('/home/conor/.data/newdata/'+file)
	elif 'coadd.fits' in file:
...
date_base = fit[0].header['MJD-OBS']
...
for file in files:
	...
	dates.append(fit[0].header['MJD-OBS'])
...
for reg in coords:
	...
	for i in range(len(f[2].data['XWIN_WORLD'])):
		if f[2].data['MAG_AUTO'][i] + magb < 50:
			...
			ra_err.append(f[2].data['ERRAWIN_IMAGE'][i]*1.012)
			dec_err.append(f[2].data['ERRBWIN_IMAGE'][i]*1.012)
...
for j in range(len(sep2d.arcsecond)):
	...
	decerr_bmatched.append(dec_baseerr[idxs[j]])
...
for i in range(len(x)):
	...
	if i!=0 and np.abs(ra_matched[i] - ra_matched[i-1]) < 0.000277778:
		...
		chunkspmra.append(slope1)
		chunkspmdec.append(m1.values['b'])
		chunkspmraerr.append(std_err1)
